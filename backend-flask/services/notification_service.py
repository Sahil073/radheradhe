...
class NotificationService:
    ...
    def send_email(self, to, subject, message, priority="NORMAL"):
        ...
        try:
            ...
            sender_password = Config.EMAIL_PASSWORD  # Add to config
    ...
    def send_emergency_alert(self, alert_type, details, affected_zones=None):
        ...
        # Send to admin via all channels
        results = {}
        ...
        # Send Firebase notification to all admin users
    ...
    def _log_notification(self, channel, recipient, message, status, priority="NORMAL"):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO alerts (alert_type, severity, message, recipient, status)
                VALUES (%s, %s, %s, %s, %s)
            """, (channel, priority, message, recipient, status))
